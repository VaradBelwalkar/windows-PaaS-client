...
try:
    ...
    r1 = client.get(base_url + "container/{}/{}/".format(sys.argv[1],sys.argv[2]))
    ...
    for link in productDivs:
        ...
        if link['href'] == "info":
            print(link.string)
            sys.exit()
    fp=open("{}\\Scripts\\keyForRemoteServer".format(projectPath),'w')
    fp.write(privateKey)
    fp.close()

    print("\n\nTrying to connect to remote container...\n\n")
    try:
       subprocess.run(["ssh","-o","StrictHostKeyChecking=no" ,"-o", "UserKnownHostsFile=/dev/null", "-i" ,"{}\Scripts\keyForRemoteServer".format(projectPath) ,"-p", "{}".format(sshPortNo), "user@{}".format(data['ipAddr'])],shell=True,check=True)
    except:
        pass
